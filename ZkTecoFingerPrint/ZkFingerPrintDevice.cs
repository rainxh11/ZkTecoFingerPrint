...
public class ZkFingerPrintDevice : IDisposable
{
    ...
    public async Task<ZkResult<ZkFingerPrintResult?>> AcquireFingerprintAsync(CancellationToken ct = default)
    {
        ...
        var response = await Task.Run(() => (ZkResponse)ZKFPM_AcquireFingerprint(Handle, pointer, (uint)buffer.Length, templatePointer, ref size), ct);
        ...
        return new ZkResult<ZkFingerPrintResult?>(response, response is ZkResponse.Ok
                                                                ? new ZkFingerPrintResult(buffer, Width, Height,
                                                                    new ZkFingerDeviceInfo
                                                                    {
                                                                        Handle = Handle,
                                                                        Name = Name,
                                                                        SerialNumber = SerialNumber
    ...
    public async Task<ZkResult<ZkFingerPrintResult?>> AcquireFingerprintAsync(
